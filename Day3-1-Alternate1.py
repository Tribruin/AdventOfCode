...
# 			print("Parallel")
		...
		if self.isVertical:
			...
			if y1 > y2:
				y1, y2 = y2, y1
			if crossSegment.p1.y in range(y1, y2+1):
				cross = True
		else:
			x1, x2 = self.p1.x, self.p2.x
			if x1 > x2:
				x1, x2 = x2, x1
			if crossSegment.p1.x in range(x1, x2+1):
				cross = True
		return cross
...
def main():		
	...
	for seg1 in wire1.segments:
		for seg2 in wire2.segments:
			print(seg1)
			print(seg2)
			print(seg1.crossPoint(seg2))
			print()

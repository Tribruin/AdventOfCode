...
# 	from itertools import permutations
	...
	def readFromLocation(self, locValue):
		...
		if mask == '0':
			...
			location = self.relativeBase + checkValue
			x = self.programCode[location]
			self.__printToLog(2, "\t\tGetting Relative Value {0} from Position {1} using Relative Base: {2} and Offset: {3}".format(x,location, self.relativeBase, location ))
		else:
			self.__printToLog(0, "*** ILLEGAL MASK {0} ***".format(mask))		
		...
		self.__printToLog(3,"\t\t\tReturn Value: {0}".format(location))
		return x, location
...
# 		Append Location store
	...
	def getInstruction(self):
		...
		self.__printToLog(1,"Step: {2} - Received Instruction Code {1} from Position {0}".format(self.currentIndex, nextInstruction, self.steps))

        ...
        # Orientation is Horizontal
            ...
            for k in range(len(wire1)-1):
                ...
                # Check for X crossing
                    if yy1 > yy2:
                    # Swap yy1 and yy2 if downwards line
                        yy1, yy2 = yy2, yy1		

                    print("    Checking Vertical Segment for crossing at Y={3} on Wire 1 at XX={0} from YY1={1} to YY2={2}".format(xx1, yy1, yy2, y1))
                    if (x1 < xx1 and xx1 < x2):
                    # Only check if x is inside the segment	
                        if (yy1 < y1 and y1 < yy2): 
                            print("        Found Crossing at ({1}, {0})".format(xx1, y1))
        ...
        # Orientation is Vertical
            ...
            for k in range(len(wire1)-1):
                ...
                # Check for Y crossing
                    if xx1 > xx2:
                    # Swap xx1 and xx2 if downwards line
                        xx1, xx2 = xx2, xx1
                    

                    print("    Checking Horizontal Segment for crossing at X={3} on Wire 1 at YY={0} from XX1={1} to XX2={2}".format(yy1, xx1, xx2, x1))
                    if (y1 < yy1 and yy1 < y2):
                    # Only check if y is inside the segment
                        if (xx1 < x1 and x1 < xx2):
                            print("        Found Crossing at ({0}, {1})".format(x1, yy1))
...
# 						print("        No X Crossing")
    ...
    # Let's walk through the wire, segment 
    allCrossPoints = []
    for i in range(len(wire2)-1):
        ...
        if x1 == x2:
            ...
            print("Checking Vertical Segment on Wire 2 at X={0} from Y1={1} to Y2={2}".format(x1, y1, y2))
            #allCrossPoints.append(checkSegment(wire2[i], wire2[i+1]))
            checkSegment(wire2[i], wire2[i+1])
        else:
            # Check Horizontal Segment
            print("Checking Horizontal Segment Wire 2 at Y={0} from X1={1} to X2={2}".format(y1, x1, x2))
# 			allCrossPoints.append(checkSegment(wire2[i], wire2[i+1]))
            checkSegment(wire2[i], wire2[i+1])

    return allCrossPoints
        
def getShortestDistance(crossPoints):
    ...
    for point in crossPoints:
        x1, y1 = point
        distance = abs(x1) + abs(y1)
        if distance < shortestDistance:
            shortestDistance = distance
            shortestPoint = point
        
    return shortestPoint, shortestDistance
...
finalCrossPoints = checkCrossPoints(wire1Array, wire2Array)
print(finalCrossPoints)

finalPoint, finalDistance = getShortestDistance(finalCrossPoints)
